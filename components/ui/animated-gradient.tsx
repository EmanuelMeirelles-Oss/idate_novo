...
/**
 * Campo de gradiente animado em WebGL2.
 *
 * Origem: 21st.dev. Três modificações em relação ao original, todas por causa
 * de problemas que o original tem em produção:
 *
 * 1. `prefers-reduced-motion` desliga o canvas por completo. Campo em
 *    movimento perpétuo é exatamente o tipo de coisa que dispara enxaqueca e
 *    desconforto vestibular em quem pede movimento reduzido.
 * 2. O laço de animação PAUSA quando o elemento sai da tela. O original roda
 *    requestAnimationFrame para sempre, queimando GPU e bateria para desenhar
 *    algo que ninguém está vendo.
 * 3. Sem WebGL2 disponível, o componente não renderiza nada em vez de deixar
 *    um canvas preto por cima do conteúdo.
 */
...
export default function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    if (!canvas || !container || !isMounted || movimentoReduzido) return;
    ...
    // Pausa o laço quando o campo sai da tela. Sem isto o rAF roda para
    // sempre, queimando GPU e bateria para desenhar o que ninguém vê.
    const observadorVisibilidade = new IntersectionObserver(
      ([entrada]) => {
        visivelRef.current = entrada.isIntersecting;
      },
      { rootMargin: "120px" },
    );
    observadorVisibilidade.observe(container);
  ...
  }, [isMounted, movimentoReduzido, params]);

  // Sob movimento reduzido não há campo: o conteúdo por cima já carrega a
  // mensagem, e um fundo parado seria só peso de GPU sem função.
